...
describe("ExtraEntryRow", () => {
  it("labels every control for assistive technology", () => {
    ...
    expect(screen.getByRole("button", { name: "Remover desconto" })).toBeInTheDocument();
  ...
  it("keeps the typed description", async () => {
    ...
    await user.type(screen.getByLabelText("Descrição do desconto"), "Plano Odonto");

    expect(screen.getByLabelText("Descrição do desconto")).toHaveValue("Plano Odonto");
  });
