...
describe("GoogleAd", () => {
  ...
  it("applies custom format and responsive values", () => {
    vi.stubEnv("NEXT_PUBLIC_ADSENSE_ID", MOCK_ADSENSE_ID);
    const { container } = render(<GoogleAd format="fluid" responsive="false" />);
    const insElement = container.querySelector("ins.adsbygoogle");
    expect(insElement?.getAttribute("data-ad-format")).toBe("fluid");
    expect(insElement?.getAttribute("data-full-width-responsive")).toBe("false");
  });
  ...
  it("pushes the ad only once when StrictMode re-invokes the mount effect", () => {
    ...
    const adsbygoogle = (window as Record<string, unknown>).adsbygoogle as Array<Record<string, unknown>>;
