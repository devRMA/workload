...
test.describe("Salary Calculator (Custo da Hora)", () => {
  ...
  test("should display default salary elements", async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Carga Horária Mensal")')).toBeVisible();
  });
  ...
  test("should add and remove extra deductions", async ({ page }) => {
    ...
    const addDeductionBtn = page.locator('button:has-text("Adicionar")').first();
    await addDeductionBtn.click();

    const deductionInput = page.getByPlaceholder("Nome (ex: Plano de Saúde)").first();
